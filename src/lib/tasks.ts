...
export async function downloadNode(projectPath: string, nodeInfo: NodeInfo, spinner: Spinner) {
  ...
  const fileDetails = await new Promise<DownloadEndedStats>((resolve, reject) => {
  ...
  if (fileDetails.incomplete) {
    throw new Error("Node download incomplete");
  }

  const nodePath = path.resolve(binPath, fileDetails.filePath);
